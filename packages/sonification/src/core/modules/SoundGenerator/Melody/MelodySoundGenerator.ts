...
export class MelodySoundGenerator extends BaseSoundGenerator {
  generate(
    data: number[],
    ...
    const dataPoints: DataPoint[] = data.map((value, index) => {
      ...
      return {
        ...
        note: this.mapValueToNoteName(normalizedValue),
      };
    });
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const frequency = this.mapValueToNote(normalizedValue);
